...
void BF_grammar::synth_analize()
{
	...
	while (curr_symb != m_in_word.end())
	{
		...
		case Less:
            ...
            *m_output << "COPY " << QString::fromStdString((*curr_symb)->m_name) << '\n';
			++curr_symb;
			break;
		case Equal:
			stack.push(*curr_symb);
		...
		case More:
			...
			while (true)
			{
				...
				if (m_BF_table[*stack.top()][*top] == Less)
				{
                    ...
                    *m_output << find->rule_number << ")" << QString::fromStdString(find->m_non_terminal->m_name) << " -> ";
                    for (auto& vect : ready_to_wrap.m_right_part)
                        *m_output << QString::fromStdString(vect->m_name) << " ";
                    *m_output << "\n";
